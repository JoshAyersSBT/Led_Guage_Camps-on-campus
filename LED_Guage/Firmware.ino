...
void loop() 
{
    ...
    if(menuOption == '1') {
    // Basic Green LED 5mm #1 - Test Code
    // The LED will turn on and fade till it is off
    for(int i=255 ; i> 0 ; i -= 5)
    {
    ledG_1.dim(i);                      // 1. Dim Led 
    ...
    ledG_1.off();                        // 3. turns off
    }
    else if(menuOption == '2') {
    // Basic Green LED 5mm #2 - Test Code
    // The LED will turn on and fade till it is off
    for(int i=255 ; i> 0 ; i -= 5)
    {
    ledG_2.dim(i);                      // 1. Dim Led 
    delay(15);                               // 2. waits 5 milliseconds (0.5 sec). Change the value in the brackets (500) for a longer or shorter delay in milliseconds.
    }                                          
    ledG_2.off();                        // 3. turns off
    }
    else if(menuOption == '3') {
    // Basic Green LED 5mm #3 - Test Code
    // The LED will turn on and fade till it is off
    for(int i=255 ; i> 0 ; i -= 5)
    {
    ledG_3.dim(i);                      // 1. Dim Led 
    delay(15);                               // 2. waits 5 milliseconds (0.5 sec). Change the value in the brackets (500) for a longer or shorter delay in milliseconds.
    }                                          
    ledG_3.off();                        // 3. turns off
    }
...
char menu()
{
    ...
    Serial.println(F("(1) Basic Green LED 5mm #1"));
    Serial.println(F("(2) Basic Green LED 5mm #2"));
    Serial.println(F("(3) Basic Green LED 5mm #3"));
    Serial.println(F("(4) Basic Red LED 5mm"));
    Serial.println(F("(5) Rotary Potentiometer - 10k Ohm, Linear"));
    ...
    while (Serial.available()) 
    {
        ...
        {   
            ...
            if(c == '1') 
    			Serial.println(F("Now Testing Basic Green LED 5mm #1"));
    		else if(c == '2') 
    			Serial.println(F("Now Testing Basic Green LED 5mm #2"));
    		else if(c == '3') 
    			Serial.println(F("Now Testing Basic Green LED 5mm #3"));
    		else if(c == '4') 
